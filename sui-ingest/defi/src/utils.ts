...
import { TokenState } from "./interface";
...
export const getPoolInfo = async (
  ...
  // Normalized the token address
  defaultPool.token0 = normalizedTokenAddress(defaultPool.token0);
  defaultPool.token1 = normalizedTokenAddress(defaultPool.token1);
...
export let tokensMetadata: Map<string, tokens> = new Map();

export const prepareTokens = async () => {
  if (tokensMetadata.size > 0) {
  ...
  tokenData.forEach((item) => {
    tokensMetadata.set(item.token_address, item);
  });
  console.timeEnd(`Cooking tokens`);

  console.log(`Done load ${tokenData.length} tokens metadata`);
};
...
export const getTokenDecimals = async (
  ...
  const cachedData = tokensMetadata.get(token);
  if (cachedData) {
    // console.log("HIT token decimal", token, cachedData);
    return cachedData?.token_decimals || 9;
  }

  const newData = await defaultFn();
  tokensMetadata.set(newData.token_address, newData); // Add to cache
  prisma.tokens
    .upsert({
      update: {},
      create: newData,
...
export const getTradeValue = async ({
  ...
  if (contractAdressCMCMapping[toAddress]) {
    ...
    return (
      toAmount * (findNearestPoint(timestamp, historicalPriceCMC)?.price || 0)
    );
  }
...
export interface INimbusTokenPrice {
  name?: string;
...
export const getNimbusDBPrice = async (
  ...
  try {
    ...
    const price = await prisma.price_feeds.findFirst({
      where: {
        token_address: normalizedTokenAddress(contractAddress),
        chain: chain,
        timestamp: {
          gte: date.subtract(6, "hour").valueOf(), // 6 hrs timeframes
          lte: date.valueOf(),
        },
      },
      orderBy: {
    ...
    if (price) {
      // console.log(`db ${contractAddress} ${price.price}`);
      return {
        name: price.token_name,
        contract_address: contractAddress,
        symbol: price.token_symbol,
        price: price.price,
        timestamp: date.unix(),
        decimals: price.token_decimals,
        source: "Nimbus",
        logo: `https://logo.getnimbus.io/api/v1/logo?address=${contractAddress}&chain=${chain}`,
        chain,
      };
    }
    ...
    // console.log("mobula query", contractAddress);
    // return await getMobulaPrice("SUI", contractAddress, timestamp);
  } catch (err) {
    console.log(err);
    return {
      name: "",
...
export const getAllOwnedObjects = async (opts: GetOwnedObjectsParams) => {
  let result: SuiObjectResponse[] = [];
  let cursor;
  while (true) {
    const data = await suiClient.getOwnedObjects({ ...opts, cursor });
