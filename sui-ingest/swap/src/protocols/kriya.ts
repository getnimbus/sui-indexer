...
export const process = async (event: SuiEvent): Promise<trade> => {
  ...
  const poolInfo = await getPoolInfo(payload.pool_id, async () => {
  ...
  const fromTokenEvent = parseWrapperObj(event.type);
  const isFromA = fromTokenEvent === poolInfo.token0;
