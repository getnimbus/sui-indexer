...
/*
 * Inspired of bloom filter implementation of GETH
 * https://ethereum.stackexchange.com/questions/17256/what-exactly-is-included-in-the-bloom-filters
...
// CreateBloom creates a bloom filter out of the give array of byte slices
func CreateBloom(input [][]byte) Bloom {
	buf := make([]byte, 6)
	var bin Bloom
	for _, b := range input {
		bin.add(b[:], buf)
